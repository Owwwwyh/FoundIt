...
- [x] **#4 Admin role + moderation dashboard** — second role with role-based auth + stats
- [x] **#5 Campus map location picker** — Leaflet.js interactive map to pick lost/found location
- [x] **#6 AI location hints** — local probabilistic scorer (temporal/frequency/category/Haversine) + optional OpenAI enrichment
...
- **2026-06-12 — Claude:** **Photo display fix + responsive pass + Forgot Password** (OW). (1) **Photos** now use `object-fit:contain` (with a backing tone) on Home cards, item detail, dashboard thumbs, match cards and the upload preview — so the *whole* item shows instead of a cropped zoom. (2) **Responsive:** added a proper mobile **hamburger menu** to the navbar (it was overflowing on small screens when logged in — "Logout" was pushed off-screen); verified the app at 375px (cards, hero, item detail, dashboard all stack cleanly) and confirmed the desktop navbar is unchanged. (3) **Forgot Password** (new): public `POST /api/forgot-password` (emails a reset link; always replies the same way so it never reveals which emails exist) and `POST /api/reset-password`; the link carries a short-lived **signed JWT** (30 min) bound to the current password hash so it is **single-use** and needs **no DB migration**. New `ForgotPassword.vue` + `ResetPassword.vue` views, a "Forgot password?" link on the login page, and a post-reset success banner. **Verified:** backend 10/10 (incl. no account enumeration, 422/400 validation, old password stops working, new one works, reused token rejected) and the full browser flow (login link → request → reset page → new password → redirected to login → new password logs in), 0 console errors. Committed + pushed to GitHub.
- **2026-06-14 — Claude:** **Advanced features #4–#6 — admin role, campus map picker, AI location hints.** **(#4 Admin role + moderation):** added a `role` column (`user`/`admin`) to `users`; the JWT now carries the role and `JwtMiddleware` exposes it; new `AdminMiddleware` (returns 403 for non-admins) guards a new `/api/admin/*` group — `AdminController` provides `stats` (totals, **resolved rate**, items by status/category, claim counts), `items` (list/filter every item), `updateItem` (force status), `destroyItem` (delete any item) and `users`. Frontend: `isAdmin` getter, an admin route guard, an "Admin" navbar link, and a new `Admin.vue` moderation dashboard (stat cards + breakdowns + item table with Mark-resolved/Reopen/Delete). Seeded `admin@example.com` / `password123`. **(#5 Campus map picker):** added `latitude`/`longitude` columns; `ItemController` validates + stores coordinates on create/update; new `LocationMap.vue` (Leaflet + OpenStreetMap, no API key) used as an editable picker in `PostItem.vue` and a read-only viewer on `ItemDetail.vue`. **(#6 AI location hints):** new `AiLocationService` — a local probabilistic scorer that ranks the user's recent places by temporal proximity, visit frequency, category/keyword affinity and optional Haversine distance clustering, with a cold-start campus fallback; results are stored on the item as JSON `ai_location_hints` and shown on the detail page (ranked list + score bars + reasons + map markers), with an owner "Refresh suggestions" button (`POST /api/items/{id}/ai-hints`). Optional OpenAI enrichment (`OPENAI_API_KEY`) adds a one-line summary with graceful offline fallback. Added `database/migration_features.sql` (idempotent, for existing DBs), updated `.env.example`, `schema.sql`, and the README (features, API table, setup, demo checklist). Verified: all changed PHP files `php -l` clean; frontend `npm run build` succeeds with Leaflet bundled.
- **2026-06-20 — Claude:** **Advanced #4/#5/#6 fixed + fully deployed & verified LIVE** (OW). Diagnosed why the three add-ons looked "coded but not live": the live AlwaysData backend was still running the **pre-#4/#5/#6 code** (login returned no `role`, items had no `latitude`/`longitude`/`ai_location_hints`, the new routes 404/405'd) **and** the live DB lacked the new columns + admin seed — the features were written correctly, just never deployed. Fixes: **(1)** `migration_features.sql` no longer hard-codes `USE foundit;` — it now targets whichever DB is selected on the CLI, so it runs safely on the live `foundit261_foundit`; tested on a throwaway DB mirroring the live old schema (adds `role`+admin, `latitude`/`longitude`/`ai_location_hints`; **idempotent**). **(2)** Found & fixed a real **500-error bug** in `AiLocationService`: `max(1, ...$places)` spread an *associative* array, so PHP 8.1+ read the string keys as named arguments and threw — AI hints would have crashed on the live PHP 8.4 too. Switched to `array_column` + `max`. **Deployed:** uploaded the 7 backend files over SSH (backups taken, `php -l` clean on remote PHP 8.4, autoloader regenerated), ran the migration against the live DB (approved), backfilled campus coords + AI hints onto the 4 seed items (guarded so the real user's item #12 was untouched), and redeployed the frontend to Vercel (bundle now ships the Admin dashboard, Leaflet picker and AI-hints UI, pointing at the live API). **Verified live — 14/14 + moderation writes:** admin login (role=admin), `/admin/stats|items|users` 200, normal user→403, no-token→401, items return lat/lng, posting with a map pin stores+displays coords, AI hints auto-generate on post, `POST /items/{id}/ai-hints` 200 (owner) / 401 (no token) / 403 (non-owner), admin force-resolve 200 + admin delete-any 204→404 + user-blocked 403. All throwaway test data deleted; live DB left at the pristine seed.
- **2026-06-20 — Claude:** **Added two reporting features.** (1) *Admin "users & their missing items" table* — new admin-only `GET /api/admin/lost-items` returns every user's lost items joined with their owner, the last location each appeared (`location`), and `days_missing` (`DATEDIFF(today, date_reported)`); `Admin.vue` renders it as a table with columns **User**, **Missing item**, **Last location it appeared**, and **Time to be found** (humanised duration, or a green "Found" badge once resolved), sorted longest-missing first, with a client-side search/status filter. (2) *Home "Most lost" podium* — new public `GET /api/lost-leaderboard` returns the top-3 most-lost categories; `Home.vue` renders a gold/silver/bronze podium (1st place centre-stage, flanked by 1st & 2nd runners-up) that hides itself when there's no data. No schema change — both use existing columns. Updated `README.md` (highlights, endpoint table, demo checklist).
- **2026-06-12 — Claude:** **Full recheck + deployed everything live + docs synced** (OW). Re-ran the whole automated backend suite — **55/55** (auth incl. forgot/reset, items + every filter, photo, matches, full claims workflow) — plus a clean frontend production build; DB left pristine. Then **deployed live:** uploaded the 3 changed backend files (`routes/api.php`, `AuthController.php`, `MailService.php`) to AlwaysData over SSH (backups taken, `php -l` clean on remote PHP 8.4) and redeployed the frontend to Vercel (re-aliased to https://foundit-app-beta.vercel.app). **Verified live:** forgot/reset endpoints respond correctly with no account enumeration and no regression on items/matches; the live JS bundle contains the Forgot-password / Reset pages and the responsive hamburger nav. Refreshed the docs (README, this log, and `HANDOFF_TO_NEW_SESSION.md`). **Everything is now LIVE — all 3 bonus features (#1 photo, #2 email, #3 smart match) plus Forgot Password and the responsive photo/nav fixes.** Remaining roadmap: bonus **#4** (admin role + moderation) and **#5** (Leaflet map picker).
